...
dt.AdjNames2<-function(LEGEND,NAMES, MODE){
  ...
  if(MODE =="PvO"){
    ...
    LEGEND$PvO.TITLE = paste0("Predicted vs. Observed Chart with Respect to <br>", td.Frmt(LEGEND$VARIABLE))
    LEGEND$LR.CURVES = paste0(LEGEND$MODELS,"_&Delta;")
    LEGEND$LR.TITLE = paste0("Discrepancy Chart with Respect to <br>", td.Frmt(LEGEND$VARIABLE))
  }else{
...
dt.CleanData <- function(DATA,NAMES){
  ...
  ## Remove lines that have 0 exposures
  Zero.xpo.ind <- DATA[,NAMES$EXPOSURE]==0
  if(any(Zero.xpo.ind)){
    cat(paste0(sum(Zero.xpo.ind)," (",percent(sum(Zero.xpo.ind)/dt.length),") zero(s) exposures were removed from ",NAMES$EXPOSURE,"\n" ))
    DATA = DATA[!Zero.xpo.ind,]
  }

  ## Remove lines that have NAs exposures
  for(i in uNAMES){
    NA.ind <- is.na(DATA[,i])
    if(any(NA.ind)){
      cat(paste0(sum(NA.ind)," (",percent(sum(NA.ind)/dt.length),") missing value(s) were removed from ",i,"\n" ))
      DATA = DATA[!NA.ind,]
    }
  }

  ## Remove lines that have 0 Predicteds
  for(i in NAMES$MODELS){
    Zero.ind <- DATA[,i]==0
    if(any(Zero.ind)){
      cat(paste0(sum(Zero.ind)," (",percent(sum(Zero.ind)/dt.length),") zero(s) were removed from ",i,"\n" ))
      DATA = DATA[!Zero.ind,]
    }
  }
  ...
  if(SPLIT.VARIANCE && !is.null(NAMES$SPLIT.BY)){
    ...
    for(i in VARIANCE[,LEGEND$VARIABLE]){
      if(is.na(i)){
        ...
        TMP <- DATA[DATA[,NAMES$VARIABLE] == i & !is.na(DATA[,NAMES$VARIABLE]) ,]
      }
