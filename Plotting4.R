...
#' @param MODE Can choose between \code{"LR"} to produce discrepency (LR Proof) or \code{"PvO"} to produce a predicted vs observed data or "ALL" to output both.
...
#' \item{SPLIT.BY}{ Name in the legend for the factor variable by which you want to split the LR proofs by(Leave \code{NULL} to keep the names in NAMES)}
#' }
#' @param CI Setting to TRUE will draw confidence bands around the average discrepency
#' @param PATH Path to which the graph will be exported to. (Default \code{NULL} will display the graph instead of exporting)
#' @param CUTS The cut points for the variable if the user wants to provide them. Leave \code{NULL} if you want auto bucket.
  ...
  for(i in 1:length(Lvls)){
    ...
    if(is.null(LEGEND$SPLIT.BY)){
      ...
      for(i in LEGEND$LR.CURVES){
        ...
        if(CI){
          Final.Chart <- add_lines(Final.Chart,x=DATA.AGG[,LEGEND$VARIABLE] , y=DATA.AGG[,i] - 2*DATA.AGG$Data_Sigma/DATA.AGG[,LEGEND$PvO.CURVES[1+k]], yaxis="y2",
                                   name = "2&sigma; CI", line = list(color = Cols[k],  width = 0 ), showlegend=F )%>%
            add_lines(x=DATA.AGG[,LEGEND$VARIABLE] , y=DATA.AGG[,i] + 2*DATA.AGG$Data_Sigma/DATA.AGG[,LEGEND$PvO.CURVES[1+k]], yaxis="y2",fill = 'tonexty', opacity=.2,
                      name = paste0("2&sigma; CI ",td.Frmt(i)), line = list(color = Cols[k],  width = 0 ) )
        }
...
#' }
#' @param CI Setting to TRUE will draw confidence bands around the observed average
#' @param PATH Path to which the graph will be exported to. (Default \code{NULL} will display the graph instead of exporting)
#' @param CUTS The cut points for the variable if the user wants to provide them. Leave \code{NULL} if you want auto bucket.
#' @param REBASE Set to TRUE if you want your predicted to be rebased to observed levels. (Defaults to TRUE)
#' @param PALETTE ColorBrewer Palette, Default is \code{'Dark2'}
...
PvO.Chart <- function(DATA,
  ...
  Cols <- brewer.pal(n = 8, name = PALETTE)
  Lvls <- unique(DATA.AGG[,LEGEND$SPLIT.BY])
  Cols1 <- rev(colorRampPalette(c("white",brewer.pal(8,PALETTE)[6]))(max(length(Lvls),length(LEGEND$PvO.CURVES))+2))
  for(i in 1:length(Lvls)){
    Cols1<-rbind(Cols1, c(rev(colorRampPalette(c("white",brewer.pal(8,PALETTE)[i]))(length(LEGEND$PvO.CURVES)+2)),
    ...
    if(is.null(LEGEND$SPLIT.BY)){
      ...
      Final.Chart <- add_lines(Final.Chart,x=DATA.AGG[,LEGEND$VARIABLE] , y=DATA.AGG[,LEGEND$PvO.CURVES[1]],yaxis="y2",
                               name = td.Frmt(LEGEND$PvO.CURVES[1]), line = list(color = Cols[1],
                                                                                 ...
                                                                                 dash = "dashdot") )
      if(CI){
        Final.Chart <- add_lines(Final.Chart,x=DATA.AGG[,LEGEND$VARIABLE] , y=DATA.AGG[,LEGEND$PvO.CURVES[1]] - 2*DATA.AGG$Data_Sigma, yaxis="y2",
                                 name = "2&sigma; CI", line = list(color = Cols[1],  width = 0 ), showlegend=F )%>%
          add_lines(x=DATA.AGG[,LEGEND$VARIABLE] , y=DATA.AGG[,LEGEND$PvO.CURVES[1]] + 2*DATA.AGG$Data_Sigma, yaxis="y2",fill = 'tonexty', opacity=.2,
                    name = "2&sigma; CI", line = list(color = Cols[1],  width = 0 ) )
      }
...
Deviation.Chart <- function(DATA,
  ...
  if(is.null(THRESHOLD)){
    for(i in 2:3){
      ...
      TMP$Error = i*TMP$Data_Sigma_Rel/TMP[,LEGEND$PvO.CURVES[1]] ## divide the sd by the mean of the data in that bucket
      TMP<-TMP[rev(order(TMP$Error)),]
      TMP$Cumul_Xpo = cumsum(TMP[,LEGEND$EXPOSURE])/sum(TMP[,LEGEND$EXPOSURE])

      if(!is.null(LEGEND$SPLIT.BY)){
        TMP <- do.call("rbind", rep(list(TMP), nLvls))
      }
  ...
  if(!is.null(THRESHOLD)){
    ...
    ind <- 1:(nrow(DATA.AGG)/nLvls)
    Deviation.Chart <- add_lines(Deviation.Chart, x = c(0,1), y = c(1,1),
                                 name = "Threshold", line = list(color = Cols[4], width = 0 ), showlegend=F ) %>%
      add_lines(x = DATA.PL$Cumul_Xpo_3SD[ind], y = DATA.PL$Error_3SD[ind], fill = 'tonexty',
                name = paste0("3&sigma; Threshold"), line = list(color = Cols[4], width = 0, shape = "spline" ) ) %>%
      add_lines(x = DATA.PL$Cumul_Xpo_2SD[ind], y = DATA.PL$Error_2SD[ind], fill = 'tonexty',
